...
TEST(Initialization, Move) {
  bigint::big_unsigned a(5);
  bigint::big_unsigned b(std::move(a));
  EXPECT_EQ(b, 5);

  bigint::big_unsigned c(5);
  bigint::big_unsigned d;
  d = std::move(c);
  EXPECT_EQ(d, 5);
}

TEST(StringConversion, Base10) {
  bigint::big_unsigned a("123456789101112");
...
TEST(StringConversion, Base2) {
  bigint::big_unsigned b("11100000100100010000110000011110011101000111000", 2);
...
TEST(Arithmetic, Addition) {
  ...
  bigint::big_unsigned a(4294967295);
  bigint::big_unsigned b(1);

  bigint::big_unsigned a_plus_b = a + b;
  EXPECT_EQ(a_plus_b.to_string(), "4294967296");
  bigint::big_unsigned a_plus_1 = a + 1u;
  EXPECT_EQ(a_plus_1.to_string(), "4294967296");
  a_plus_1 = 1u + a;
  EXPECT_EQ(a_plus_1.to_string(), "4294967296");
  ...
  c += 1;
  EXPECT_EQ(c.to_string(), "4294967296");

  EXPECT_EQ((a++).to_string(), "4294967295");
  EXPECT_EQ(a.to_string(), "4294967296");
  EXPECT_EQ((++a).to_string(), "4294967297");

  bigint::big_unsigned d(0);
  bigint::big_unsigned e(0);
...
TEST(Arithmetic, Subtraction) {
  // Exactly at the overflow boundary for a classical 32 bit unsigned int.
  bigint::big_unsigned a("4294967296");
  bigint::big_unsigned b(1);
  bigint::big_unsigned c;
...
TEST(Arithmetic, Division) {
  bigint::big_unsigned fac50("3041409320171337804361260816606476884437764156896"
      "0512000000000000");
  bigint::big_unsigned fac100("870978248908948007941659016194448586556972064394"
  ...
  bigint::big_unsigned a = fac50 / fac100;
  EXPECT_EQ(a.to_string(), "0");
  bigint::big_unsigned b = fac100 / fac50;
...
TEST(Operators, Comparison) {
  bigint::big_unsigned a("12345678910111213141516171819");
  bigint::big_unsigned b("19181716151413121110987654321");
  bigint::big_unsigned c("12345678910111213141516171819");
  EXPECT_TRUE(a == a);
  EXPECT_TRUE(a == c);
  EXPECT_TRUE(a != b);
  EXPECT_TRUE(a < b);
...
TEST(Operators, Logic) {
  bigint::big_unsigned zero(0);
  bigint::big_unsigned one(1);
